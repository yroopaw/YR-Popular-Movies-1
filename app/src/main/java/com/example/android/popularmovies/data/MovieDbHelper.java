...
public class MovieDbHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
      ...
      /*  final String SQL_CREATE_TRAILERREVIEWS_TABLE = "CREATE TABLE " + TrailerReviewDbEntry.TABLE_NAME + " (" +
        ...
        final String SQL_CREATE_HIGHESTRATEDMOVIES_TABLE = "CREATE TABLE " + HighestRatedMovies.TABLE_NAME + " (" +


                HighestRatedMovies._ID + " INTEGER PRIMARY KEY AUTOINCREMENT," +

                // the ID of the location entry associated with this weather data
                HighestRatedMovies.COLUMN_MOVIE_ID + " STRING NOT NULL, " + /*check if int*/
                HighestRatedMovies.COLUMN_POPULARITY + " INTEGER NOT NULL, " +
                HighestRatedMovies.COLUMN_ORIGINAL_TITLE + " STRING NOT NULL, " +
                HighestRatedMovies.COLUMN_PLOT_SYNOPSIS + " STRING NOT NULL," +

                HighestRatedMovies.COLUMN_USER_RATING + " REAL NOT NULL, " +
                HighestRatedMovies.COLUMN_RELEASE_DATE + " STRING NOT NULL, " +

                HighestRatedMovies.COLUMN_POSTER_PATH_THUMBNAIL + " STRING NOT NULL, " +


                // To assure the application have just one weather entry per day
                // per location, it's created a UNIQUE constraint with REPLACE strategy
                " UNIQUE (" + HighestRatedMovies.COLUMN_MOVIE_ID +
               /* MovieListEntry.COLUMN_LOC_KEY + */ ") ON CONFLICT REPLACE);";

        sqLiteDatabase.execSQL(SQL_CREATE_HIGHESTRATEDMOVIES_TABLE);

        final String SQL_CREATE_FAVORITEMOVIES_TABLE = "CREATE TABLE " + FavoriteMovies.TABLE_NAME + " (" +


                FavoriteMovies._ID + " INTEGER PRIMARY KEY AUTOINCREMENT," +

                // the ID of the location entry associated with this weather data
                FavoriteMovies.COLUMN_MOVIE_ID + " STRING NOT NULL, " + /*check if int*/
                FavoriteMovies.COLUMN_POPULARITY + " INTEGER NOT NULL, " +
                FavoriteMovies.COLUMN_ORIGINAL_TITLE + " STRING NOT NULL, " +
                FavoriteMovies.COLUMN_PLOT_SYNOPSIS + " STRING NOT NULL," +

                FavoriteMovies.COLUMN_USER_RATING + " REAL NOT NULL, " +
                FavoriteMovies.COLUMN_RELEASE_DATE + " STRING NOT NULL, " +

                FavoriteMovies.COLUMN_POSTER_PATH_THUMBNAIL + " STRING NOT NULL, " +


                // To assure the application have just one weather entry per day
                // per location, it's created a UNIQUE constraint with REPLACE strategy
                " UNIQUE (" + FavoriteMovies.COLUMN_MOVIE_ID +
               /* MovieListEntry.COLUMN_LOC_KEY + */ ") ON CONFLICT REPLACE);";
    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int oldVersion, int newVersion) {
        ...
        sqLiteDatabase.execSQL("DROP TABLE IF EXISTS " + MovieContract.HighestRatedMovies.TABLE_NAME);
